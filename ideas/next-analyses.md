...
over program-graph edges with different filters."  The CozoDB-backed fact graph
ships with a Datalog engine, so these become rule sets rather than separate
implementations:
...
policy engine for protocol definitions, CFGs for must-close-on-all-paths.
...
### 4. Advanced Change Impact Analysis
...
### 5. Incremental / Demand-Driven Analysis
...
### 6. None/Optional Abstract Domain
...
**Reuses:** Taint engine (lattice structure maps directly), type oracle, CFGs.
**Enables:** None-safety checking without full type annotations.
**Effort:** Medium.

### 7. LLM-Guided False Positive Reduction
...
graphs.  They ship as example rule sets for `emend query` / the CozoDB engine:
...
|---|-----------|--------|-----------|--------|
| 1 | Typestate analysis | **TODO** | taint engine, CFGs | Medium |
| 2 | API migration | **TODO** | mapping store, batch | Medium |
| 3 | Spec mining | **TODO** | pattern matching | Low-medium |
| 4 | Adv. impact analysis | **TODO** | impact, git | Low-medium |
| 5 | Incremental analysis | **TODO** | parse.db, editor | Medium |
| 6 | None/Optional domain | **TODO** | taint engine, CFGs | Medium |
| 7 | LLM false positive filtering | **TODO** | MCP server | Low |
