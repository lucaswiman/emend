...
| `location_resolver.py` | Pattern match → exact CFG location resolver: `ResolvedLocation` (frozen dataclass), `LocationResolver` (two strategies: FactGraph-backed via `from_fact_graph()`, on-the-fly via `from_source()`), `MODULE_LEVEL_FUNC`/`MODULE_LEVEL_BLOCK` sentinels replacing ad-hoc `("", -1)` |
| `fact_graph.py` | Relational fact model: `SymbolFact`, `CallFact`, `ReferenceFact`, `TraceFlowFact`, `TypeFact`, `ImportFact`, `CfgEdgeFact`, `DefUseFact` (with `kind`: read/write/aug_write/del), `MethodCallFact`, `CfgBlockFact`, `DecoratorOnFact`, `SourceLocFact`, `FuncSummaryFact`, `EntryPointDecoratorFact`, `EntryPointNameFact`; `FactGraph` with indexed queries, transitive closures, `update_files()` (incremental per-file delete-then-insert), `remove_files()` (per-file fact deletion across all relations), `build_from_files()` (delegates to `update_files()`), `build_from_project()` (populates CFG blocks, decorators, source locations, block-tagged references, method calls), JSON serialization; `TraceDatalogConfig` dataclass for Datalog query configuration; Datalog query methods: `refs_datalog()`, `callers_datalog()`, `callees_datalog()`, `graph_datalog()`, `dead_code_unified()`, `unreachable_blocks_datalog()`, `trace_propagation_datalog()` (with per-variable `sanitizer_var` relation, `effect_sinks` for writes/reads predicates, `scope_kills` for scope boundary sanitization via `scope_kill` inline relation), `interprocedural_trace_datalog()`, `flow_rule_check_datalog()` |
...
| `rewrite_engine.py` | Experimental equality saturation: `EGraph`, `ENode`, `RewriteRule`, `SaturationResult`; `run_saturation()`, `load_rewrite_rules()`; loads from `.emend/rewrites.yaml` |
| `union_find.py` | `UnionFind` (disjoint set) data structure; used by `rewrite_engine.py` and `duplicate.py` |
| `type_oracle.py` | Type inference adapter: `TypeOracle` ABC + `PyreflyAdapter`, `PyrightAdapter`, `TyAdapter`, `TypeScriptAdapter` (batch via Node.js Compiler API), `RustAnalyzerAdapter` (LSP-based); `parse_type_string` (handles Python `[]`, TS `<>`/`[]` shorthand, Rust `&`/lifetime/`<>` generics, TS `=>` arrow functions), `TypeDescriptor`, `FileTypes`, `TypeBinding`, `create_type_oracle`, `detect_type_engine` (per-file extension + project config autodetection for Python/TS/Rust); results cached in `parse.db` (`type_cache` table) |
...
| `test_location_resolver.py` | Phase 7: `LocationResolver`, `ResolvedLocation`, FactGraph-backed and source-based resolution, nested functions, same-line blocks, module-level matches |
| `test_datalog_trace.py` | Phase 6: `_run_trace_datalog()` smoke tests, effect sink extraction, sanitizer quantifier, `_resolve_match_to_location`, `interprocedural_trace_datalog` config-driven |
...
| `test_type_oracle.py` | `TypeOracle` unit tests: `parse_type_string` (Python/TS/Rust types, angle brackets, array shorthand, references, arrow functions), `TypeDescriptor`, `FileTypes`, cache, parsers, `detect_type_engine` (cross-language with file_path), `TypeScriptAdapter`, `RustAnalyzerAdapter`, `create_type_oracle` cross-language factory, stress tests, optional pyrefly/pyright/typescript/rust-analyzer integration |
...
| `test_trace.py` | `trace` command (source-to-sink detection, sanitizers, propagation, config loading, traces) |
...
| `trace` | Trace analysis: tracks value flow from sources to sinks (`--config`, `--label`, `--trace`, `--json`, `--project`, `--interprocedural` for cross-function tracking with fixed-point iteration, `--max-iterations`, `--preset` for framework-specific rules) |
...
| `facts` | Query the relational fact graph for code invariants (`--type symbols\|calls\|references\|trace_flows\|types\|imports`, `--name`, `--kind`, `--file`, `--symbol`, `--label`, `--transitive`, `--json`) |
...
| `types` | Show inferred types for symbols in a file (`--name`, `--kind`, `--definitions-only`, `--json`, `--engine`). Engines: `pyrefly`/`pyright`/`ty` (Python), `typescript` (TS/JS via Node.js Compiler API), `rust-analyzer` (Rust via LSP), `auto` (default, detects from file extension and project config) |
...
- Flow rules: `flows-from` + `flows-to` + optional `not-through` for intraprocedural trace-based linting (checked by `_check_flow_rule()`)
