...
pub struct Reference {
    ...
    /// Whether scope analysis found a local, import, or builtin target.
    pub resolved: bool,
    /// Stable identity of the exact lexical import binding, when applicable.
    pub import_binding_id: Option<String>,
...
pub struct FileScope {
    ...
    /// All import bindings with lexical provenance; unlike `imports`, this
    /// retains shadowed bindings in nested scopes.
    pub scoped_imports: Vec<ScopedImportBinding>,
...
/// One import binding at an exact source/scope location.
#[derive(Debug, Clone)]
pub struct ScopedImportBinding {
...
///
/// Captures the full statement structure so Python code does not need
/// to re-parse with `ast.parse()` to split or retarget imports.
#[derive(Debug, Clone)]
...
pub struct PatternMatchingSection {
    ...
    /// Field names whose subtrees are type-annotation context (e.g.
    /// ``["type", "return_type"]`` for Python's parameter types and
    /// return types).  References within these fields get
    /// ``in_annotation = true``.
    #[serde(default)]
    pub annotation_fields: Vec<String>,
    #[serde(default)]
...
pub struct ExportsSection {
    ...
    #[serde(default)]
    pub export_statement: Option<String>,
    #[serde(default)]
...
impl ScopeResolver {
    ...
    fn build_file_scope(
        ...
        if self.config.language.name == "rust" {
            ctx.imports.clear();
            ctx.scoped_imports.clear();
            for (binding, line, byte_offset) in self.collect_rust_imports(tree, source) {
                let scope_id = ctx.scopes.iter()
                    .filter(|scope| {
                        scope.start_byte <= byte_offset && byte_offset < scope.end_byte
                    })
                    .min_by_key(|scope| scope.end_byte.saturating_sub(scope.start_byte))
                    .map_or(module_id, |scope| scope.id);
                ctx.add_import(scope_id, byte_offset, line, binding);
            }
        }

        // Second pass: resolve all identifier/attribute references to QNs.
        let (references, all_qnames) = self.collect_file_references(
            tree, path, source, &module_path, &ctx.scopes, &ctx.scope_index,
            &ctx.scoped_imports,
        );
        ...
        FileScope {
            ...
            scopes: ctx.scopes,
            imports: ctx.imports,
            scoped_imports: ctx.scoped_imports,
    ...
    fn walk_references(
        ...
        let is_from_import = !cfg_imports.import_from.is_empty()
            ...
            || cfg_imports.fixed_modules.contains_key(node_kind);
        ...
        if (is_attribute || is_scoped_path) && !self.is_qualified_component(&node) {
            if let Some(full_name) = self.collect_dotted_name(&node, source) {
                ...
                let resolved = receiver_qn
                    .map(|qn| (qn, None))
                    .or_else(|| self.resolve_dotted_name_detail(
        ...
        if cursor.goto_first_child() {
            loop {
                ...
                // Check if this child is in an annotation context.
                // A child is an annotation if it is a field listed in
                // config.pattern_matching.annotation_fields on the parent.
                let child_in_annotation = in_annotation || {
                    let ann_fields = &self.config.pattern_matching.annotation_fields;
                    !ann_fields.is_empty() && ann_fields.iter().any(|f| {
                        node.child_by_field_name(f.as_str()) == Some(child)
                    })
                };
    ...
    fn visible_import<'a>(
        ...
        imports
            .iter()
            .filter(|import| {
                import.binding.local_name == name
                    && import.byte_offset <= byte_offset
                    && ancestors.contains(&import.scope_id)
            })
            .max_by_key(|import| {
                let distance = ancestors
                    .iter()
                    .position(|scope| *scope == import.scope_id)
                    .unwrap_or(usize::MAX);
                (Reverse(distance), import.byte_offset)
            })
    }

    fn resolve_identifier_detail(
        &self,
        name: &str,
        byte_offset: usize,
        module_path: &str,
        ...
        if let Some(scoped) = self.visible_import(
            name, byte_offset, scope_id, scopes, scope_index, imports,
        ) {
            let imp = &scoped.binding;
            if imp.is_star && imp.local_name == "*" {
        ...
        if self.config.builtins.names.iter().any(|b| b == name) {
            return Some((format!("builtins.{}", name), None));
        }
    ...
    fn resolve_dotted_name_detail(
        ...
        if parts.len() < 2 {
            return self.resolve_identifier_detail(
                dotted, _byte_offset, module_path, scopes, scope_index, imports, scope_id,
            );
        }
        let root = parts[0];
        let rest = parts[1];
        ...
        if let Some(scoped) = self.visible_import(
            root, _byte_offset, scope_id, scopes, scope_index, imports,
        ) {
            let imp = &scoped.binding;
            if imp.is_star && imp.local_name == "*" {
                return None;
    ...
    pub(crate) fn resolve_receiver_method(
            ...
            && receiver.kind() == cfg.self_receiver_node
        {
            ...
            self.resolve_identifier_detail(&nominal, node.start_byte(), module_path,
                scopes, scope_index, imports, scope_id)?.0
        };
    ...
    fn walk_node(
        ...
        if node_kind == self.config.imports.import_statement
            ...
            || self.config.imports.fixed_modules.contains_key(node_kind)
        {
            self.collect_import(ctx, &node, scope_for_children);
        }
    ...
    fn collect_import(
        ...
        let is_from_import = (!imports.import_from.is_empty()
            ...
            || imports.fixed_modules.contains_key(node_kind);
        ...
        if is_plain_import {
            ...
            let module_node = node
                ...
                ;
            let module_path = imports.fixed_modules.get(node_kind).cloned()
                .or_else(|| module_node.map(|n| ctx.text(n).to_string()))
                .unwrap_or_default();
            let module_node_id = module_node.map(|node| node.id());

            let mut found_any = false;
            for_each_child(node, |child| {
                if module_node_id == Some(child.id()) {
                    return;
                }
                let ck = child.kind();
            ...
            if !found_any && !module_path.is_empty() {
                ...
                Self::collect_nested_import_names(
                    node, ident_kind, &module_path, scope_id, byte_offset, line, ctx,
                );
            }
        }
    }
        ...
        loop {
            ...
            if child.kind() == "import_specifier" {
                let imported = child
                    ...
                    .map(|name| ctx.text(name).to_string());
                if let Some(imported) = imported {
                    let local_name = child
                        .child_by_field_name("alias")
                        .map(|alias| ctx.text(alias).to_string())
                        .unwrap_or_else(|| imported.clone());
                    ctx.add_import(scope_id, byte_offset, line, ImportBinding {
                        local_name,
                ...
                if let Some(name) = child.named_child(0) {
                    ctx.add_import(scope_id, byte_offset, line, ImportBinding {
                        local_name: ctx.text(name).to_string(),
                        module_path: module_path.to_string(),
                        imported_name: Some("*".to_string()),
                        is_star: true,
                    });
                }
            } else if child.kind() == ident_kind && node.kind() == "import_clause" {
                ...
                // Recurse into child
                Self::collect_nested_import_names(
                    &child, ident_kind, module_path, scope_id, byte_offset, line, ctx,
                );
            }
            if !cursor.goto_next_sibling() {
                break;
    ...
    /// Collect structured import statements from a parsed tree.
    ///
    /// Unlike `collect_import` (which populates `ImportBinding`s for scope
    /// resolution), this returns a list of [`StructuredImport`]s that
    /// capture the full statement structure — module, imported names with
    /// aliases, byte ranges — so callers can split or retarget imports
    /// without re-parsing with a language-specific AST library.
    pub fn collect_structured_imports(
        &self,
        tree: &tree_sitter::Tree,
        source: &str,
    ) -> Vec<StructuredImport> {
        let source_bytes = source.as_bytes();
        let imports = &self.config.imports;
        let mut result = Vec::new();
        ...
        fn walk_for_imports(
            ...
            source: &[u8],
            imports: &ImportsSection,
            result: &mut Vec<StructuredImport>,
        ) {
            let nk = node.kind();
                ...
                || imports.fixed_modules.contains_key(nk)
            {
                ...
                // Count leading dots for relative imports.  In some grammars
                // (e.g. Python) the dots are embedded in the module_name text
                // (e.g. ".helpers"); in others they may be separate child
                // nodes.  We check both.
                let mut level: usize = raw_module.chars().take_while(|ch| *ch == '.').count();
                if level == 0 {
                    let mut c = node.walk();
                    for child in node.children(&mut c) {
                        if child.kind() == "." || child.kind() == "import_prefix" {
                            let dots = node_text(child, source);
                            level += dots.chars().filter(|ch| *ch == '.').count();
                        }
                    }
                }
            ...
            // Recurse into children to find nested import statements
            // (e.g. inside if TYPE_CHECKING blocks).
            let mut cursor = node.walk();
            if cursor.goto_first_child() {
                loop {
                    walk_for_imports(cursor.node(), source, imports, result);
                    if !cursor.goto_next_sibling() {
                        break;
                    }
                }
    ...
    pub fn collect_rust_imports(
        ...
        fn expand_use_tree(
            ...
            line: usize,
            byte_offset: usize,
            result: &mut Vec<(ImportBinding, usize, usize)>,
        ) {
            match node.kind() {
                ...
                "scoped_use_list" => {
                    ...
                    if let Some(list_node) = node.child_by_field_name("list") {
                        expand_use_tree(list_node, &new_prefix, src, line, byte_offset, result);
                    }
                }
                        ...
                        loop {
                            ...
                            if ck != "{" && ck != "}" && ck != "," {
                                if ck == "self" {
                                    ...
                                    expand_use_tree(child, prefix, src, line, byte_offset, result);
                                }
                            }
                            if !cursor.goto_next_sibling() {
                                break;
        ...
        // Walk the full tree so block-local bindings retain their lexical scope.
        fn walk_imports(
            node: tree_sitter::Node,
            src: &[u8],
            result: &mut Vec<(ImportBinding, usize, usize)>,
            ...
            if nk == "use_declaration" {
                ...
                if let Some(arg) = node.child_by_field_name("argument") {
                    expand_use_tree(arg, "", src, line, node.start_byte(), result);
                }
                return;
            }
            ...
            // Recurse into lexical containers; scope assignment happens after
            // collection from the occurrence's source position.
            let mut cursor = node.walk();
            if cursor.goto_first_child() {
                loop {
                    walk_imports(cursor.node(), src, result);
                    if !cursor.goto_next_sibling() {
                        break;
                    }
                }
        ...
        walk_imports(tree.root_node(), source_bytes, &mut result);
        result
    }
    ...
    pub fn python_default() -> Self {
        LanguageConfig {
            ...
            pattern_matching: PatternMatchingSection {
                ...
                annotation_fields: vec!["type".to_string(), "return_type".to_string()],
            ...
            cfg: CfgSection {
                function_nodes: vec!["function_definition".to_string(), "async_function_definition".to_string()],
                decorated_node: "decorated_definition".to_string(),
                container_nodes: vec!["class_definition".to_string()],
                definition_nodes: vec![
                    "function_definition".to_string(), "async_function_definition".to_string(),
                    "class_definition".to_string(), "decorated_definition".to_string(),
                ],
                block_nodes: vec!["block".to_string(), "module".to_string()],
                if_nodes: vec!["if_statement".to_string()],
                if_style: "children".to_string(),
                elif_clause: "elif_clause".to_string(),
                else_clause: "else_clause".to_string(),
                for_nodes: vec!["for_statement".to_string()],
                for_variable_field: "left".to_string(),
                for_iterable_field: "right".to_string(),
                for_has_else: true,
                while_nodes: vec!["while_statement".to_string()],
                while_has_else: true,
                try_nodes: vec!["try_statement".to_string()],
                try_style: "children".to_string(),
                try_body_node: "block".to_string(),
                except_clauses: vec!["except_clause".to_string(), "except_group_clause".to_string()],
                try_else_clause: "else_clause".to_string(),
                try_finally_clause: "finally_clause".to_string(),
                with_nodes: vec!["with_statement".to_string()],
                match_nodes: vec!["match_statement".to_string()],
                match_subject_field: "subject".to_string(),
                case_clause: "case_clause".to_string(),
                case_body_node: "block".to_string(),
                return_nodes: vec!["return_statement".to_string()],
                throw_nodes: vec!["raise_statement".to_string()],
                break_nodes: vec!["break_statement".to_string()],
                continue_nodes: vec!["continue_statement".to_string()],
                assert_nodes: vec!["assert_statement".to_string()],
                condition_field: "condition".to_string(),
                consequence_field: "consequence".to_string(),
                body_field: "body".to_string(),
                expression_statement_node: "expression_statement".to_string(),
                identifier_node: "identifier".to_string(),
