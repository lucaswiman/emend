...
def _finish_transform(
    ...
    *,
    ...
    return _generate_diff(str(file_path), source_code, new_code)
...
def _generate_diff(file_path: str, old_code: str, new_code: str) -> str:
    ...
    return "".join(
        difflib.unified_diff(
            old_lines, new_lines, fromfile=file_path, tofile=file_path
        )
    )
    ...
    if selector.accessor is not None:
        ...
        if i < len(source_bytes) and source_bytes[i : i + 1] == b",":
            ...
            if i > 0 and source_bytes[i - 1 : i] == b",":
                ...
                if selector.component == "decorators":
                    i = start_byte
                    while i > 0 and source_bytes[i - 1 : i] not in (
                        b"\n",
                        b"\r",
                        b"@",
                    ):
                    ...
                    j = end_byte
                    while j < len(source_bytes) and source_bytes[j : j + 1] in (
                        b" ",
                        b"\t",
                    ):
                        j += 1
                    if j < len(source_bytes) and source_bytes[j : j + 1] in (
                        b"\n",
                        b"\r",
                    ):
                        j += 1
...
def _extract_string_content_from_text(text: str, ext: str = "py") -> str | None:
    """Return a string literal's inner content, or None for other text."""
