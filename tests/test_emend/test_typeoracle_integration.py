...
@pytest.mark.parametrize("source, pattern, positions, expected", [
    ("sink(sink)\n", "sink($X:type[str])", [(1, 1, "int"), (1, 6, "str")], 1),
    ("x; sink(x)\n", "sink($X:type[str])", [(1, 1, "int"), (1, 9, "str")], 1),
    ("sink(\n  x\n)\n", "sink($X:type[str])", [(2, 3, "str")], 1),
    ("é; sink(x)\n", "sink($X:type[str])", [(1, 10, "str")], 1),
    ("sink(x, x)\n", "sink($X:type[str], $X)", [(1, 6, "str"), (1, 9, "int")], 0),
])
def test_typed_captures_use_actual_occurrences(tmp_path, source, pattern, positions, expected):
    ...
    types = _build_file_types(str(path), [
        TypeBinding("x", line, col, col + 1, TypeDescriptor.named(typ), typ, "reference")
        for line, col, typ in positions
    ])
