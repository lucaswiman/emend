...
class TestRunChecksEngine:
    ...
    @staticmethod
    def _run_unified_rule(
        tmp_path, *, source="def f():\n    eval('code')\n", mode=None,
    ):
        from emend.checks.engine import run_checks
        ...
        return run_checks([str(test_file)], config=config_file, mode=mode)
    ...
    def test_unified_rule_not_double_reported(self, tmp_path):
        ...
        """
        violations = self._run_unified_rule(tmp_path)
    ...
    @pytest.mark.parametrize("mode", ["policy", "lint"])
    def test_standalone_mode_still_reports_unified_rule(self, tmp_path, mode):
        violations = self._run_unified_rule(tmp_path, mode=mode)
        assert any(v.rule_name == "no-eval" for v in violations)
    ...
    def test_multiple_matches_same_line_preserved(self, tmp_path):
        ...
        violations = self._run_unified_rule(
            tmp_path, source="x = eval('a'); y = eval('b')\n",
        )
        no_eval = [v for v in violations if v.rule_name == "no-eval"]
